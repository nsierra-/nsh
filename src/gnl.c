...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2014/03/19 03:41:30 by nsierra-          #+#    #+#             */
/*   Updated: 2014/03/23 19:37:46 by nsierra-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_read		*ft_newread(int fd)
{
	...
	if ((ret = read(fd, tmp, (size_t)BUFF_SIZE)) <= 0)
	{
		free(red);
		free(tmp);
		return (NULL);
...
static	int			ft_printgnl(int n, t_read **tab, t_read **s, char **l)
{
	char			*tmpstr;
	int				index;
...
int					get_next_line(int fd, char **line)
{
	...
	tab[0] = red;
	tab[1] = prevtmp;
	return (ft_printgnl(ft_findendl(fd, red), tab, &start, line));
}
